...
public class PirateHealth : MonoBehaviour
{
    ...
    private void Start()
    {
        maxHealth = Mathf.RoundToInt(BalanceVariables.pirateEnemy["maxHealth"]);
        health = maxHealth;
    }
    ...
    public void SetHealth(int newHealth)
    {
        if(newHealth > 0)
        {
            if(newHealth < BalanceVariables.pirateEnemy["maxHealth"])
            {
                health = newHealth;
            }
            else
            {
                health = Mathf.RoundToInt(BalanceVariables.pirateEnemy["maxHealth"]);
            }
        }  
    }
